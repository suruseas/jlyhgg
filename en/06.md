...
It turns out that if you want to define computations 
by defining what stuff is instead of defining steps that change some state and maybe looping them,
higher order functions are indispensable.
They're a really powerful way of solving problems and thinking about programs.



## <a name="6.1">Curried functions</a>
...
Doing max 4 5 first creates a function 
that takes a parameter and returns either 4 or that parameter, depending on which is bigger.
...
<img src="//s3.amazonaws.com/lyah/curry.png" alt="挿絵 haskell curry" class="img-left">
...
we get back a partially applied function, meaning a function that takes as many parameters as we left out.
Using partial application (calling functions with too few parameters,
if you will) is a neat way to create functions on the fly 
...
Yo! Make sure you really understand how curried functions and partial application work 
...
## <a name="6.2">Some higher-orderism is in order</a>
...
<img src="//s3.amazonaws.com/lyah/bonus.png" alt="挿絵 rocktopus" class="img-right">
...
but to save ourselves a headache, we'll just say that this function takes two parameters and returns one thing.
...
> despite each function actually taking only one parameter and returning partially applied functions 
...
the joining function, but that argument doesn't matter in the edge conditions, so we just use a _ for it.
...
while loops, setting something to a variable, checking its state, etc.
...
the second pair of parentheses is really unnecessary, because -> is right associative by default.
(a -> b -> c) -> (b -> a -> c) 
is the same as (a -> b -> c) -> (b -> (a -> c)), which is the same as (a -> b -> c) -> b -> a -> c.
...
by thinking ahead and writing what their end result would be if they were called fully applied.
...
that it takes a function that takes an a and returns a b, a list of a's and returns a list of b's.
...
However, using map is much more readable for cases where you only apply some function to the elements of a list,
...
We used list comprehensions to filter out the list elements 
that are smaller than (or equal to) and larger than the pivot.
...
<img src="//s3.amazonaws.com/lyah/map.png" alt="挿絵 map" class="img-left">
...
we would have solved it by nesting three loops and then testing if the current combination 
satisfies a right triangle and if it has the right perimeter.
...
If we wanted to get the first word of the string 
"elephants know how to party",
we could do takeWhile (/=' ') "elephants know how to party" 
...
Awesome!
We start with some initial data (the infinite list of all natural numbers) 
and then we map over it, filter it and cut it until it suits our needs and then we just sum it up.
...
Only when we force Haskell to show us the sum does the sum function 
say to the takeWhile that it needs those numbers.
...
Yay! It seems to be working correctly. 
...
> Note: This function has a type of 
> numLongChains :: Int because length returns an Int instead of a Num a for historical reasons.
...
if not for any other reason than to illustrate how currying works and how 
(partially applied)
 functions are real values that you can pass around to other functions or put into lists 
(you just can't turn them to strings).
So far, we've only mapped functions that take one parameter over lists,
like map (*2) [0..] to get a list of type (Num a) => [a], but we can also do map (*) [0..] without a problem.
...
## <a name="6.4">Lambdas</a>

<img src="//s3.amazonaws.com/lyah/lambda.png" alt="挿絵 lambda" class="img-right">

Lambdas are basically anonymous functions that are used because we need some functions only once.
Normally, we make a lambda with the sole purpose of passing it to a higher-order function.
To make a lambda, 
we write a \ (because it kind of looks like the greek letter lambda 
if you squint hard enough) and then we write the parameters, separated by spaces.
...
If you look about 5 inches up,
you'll see that we used a where binding in our numLongChains function 
...
The expression (\xs -> length xs > 15) 
returns a function that tells us whether the length of the list passed to it is greater than 15.

<img src="//s3.amazonaws.com/lyah/lamb.png" alt="挿絵 lamb" class="img-left">
...
## <a name="6.5">Only folds and horses</a>

<img src="//s3.amazonaws.com/lyah/origami.png" alt="挿絵 folded bird" class="img-right">
...
We'd introduce the x:xs pattern and then we'd do some action 
that involves a single element and the rest of the list.
...
only this time, we'll use a fold instead of explicit recursion.
...
<img src="//s3.amazonaws.com/lyah/foldl.png" alt="挿絵 foldl" class="img-left">
...
Now first, 0 is used as the acc parameter to the binary function 
and 3 is used as the x (or the current element) parameter.
...
I'm sure you all know that elem checks whether a value is part of a list 
so I won't go into that again (whoops, just did!).
Let's implement it with a left fold.

    elem' :: (Eq a) => a -> [a] -> Bool
    elem' y ys = foldl (\acc x -> if x == y then True else acc) False ys

Well, well, well, what do we have here? The starting value and accumulator here is a boolean value.
...
<img src="//s3.amazonaws.com/lyah/washmachine.png" alt="挿絵 fold this up!" class="img-right">
...
but this just goes to show, you can still achieve it by using folds.
...
but because we're interested in how the sum progresses, we're going to do a scan.
...
## <a name="6.6">Function application with $</a>
...
<img src="//s3.amazonaws.com/lyah/dollar.png" alt="挿絵 dollar" class="img-left">

What the heck?
What is this useless operator?
It's just function application! Well, almost, but not quite! Whereas normal function application 
(putting a space between two things)
...
we'd have to write sqrt (3 + 4 + 9) or if we use $ 
we can write it as sqrt $ 3 + 4 + 9 because $ has the lowest precedence of any operator.
That's why you can imagine a $ being sort of the equivalent of writing an opening parentheses 
...
## <a name="6.7">Function composition</a>
...
when called with a parameter, say, x is the equivalent of calling g with the parameter x 
and then calling the f with that result.

In Haskell, function composition is pretty much the same thing.
We do function composition with the .
function, which is defined like so:

<img src="//s3.amazonaws.com/lyah/notes.png" alt="挿絵 notes" class="img-left">

Mind the type declaration.
f must take as its parameter a value that has the same type as g's return value.
So the resulting function takes a parameter of the same type 
that g takes and returns a value of the same type that f returns.
The expression negate .
(* 3) returns a function that takes a number, multiplies it by 3 and then negates it.
...
Fabulous! Function composition is right-associative, so we can compose many functions at a time.
...
(also called the pointless style).
...
and what kind of functions composing them results in instead of thinking about data and how it's shuffled around.
...
The prefered style is to use let bindings to give labels to intermediary results 
or split the problem into sub-problems and then put it together 
