...
## <a name="7.1">Loading modules</a>

<img src="//s3.amazonaws.com/lyah/modules.png" alt="挿絵 modules" class="img-right">

A Haskell module is a collection of related functions, types and typeclasses.
A Haskell program is a collection of modules where the main module loads up the other modules 
...
If your own code is separated into self-contained modules which don't rely on each other too much 
(we also say they are loosely coupled),
...
Composing length and nub by doing length .
nub produces a function that's the equivalent of \xs -> length (nub xs).
...
Say we already have our own function 
that's called nub and we want to import all the functions from Data.List except the nub function:
...
we have to do Data.Map.filter, whereas just filter still refers to the normal filter we all know and love.
...
A great way to pick up new Haskell knowledge is 
to just click through the standard library reference and explore the modules and their functions.
...
because it doesn't clash with any Prelude names except for those that Prelude already steals from Data.List.
...
the third powers are then in the first row, the second powers in the second one and so on.
Mapping sum to that produces our desired result.

<img src="//s3.amazonaws.com/lyah/legolists.png" alt="挿絵 shopping lists" class="img-left">
...
What actually happens is that the accumulator kind of makes a promise that it will compute its value 
when asked to actually produce the result (also called a thunk).
That happens for every intermediate accumulator and all those thunks overflow your stack.
The strict folds aren't lazy buggers and actually compute the intermediate values 
as they go along instead of filling up your stack with thunks.
...
So if you want to completely flatten [[[2,3],[3,4,5],[2]],[[2,3],[3,4]]], which is a list of lists of lists,
...
while the predicate holds and then when an element is encountered that doesn't satisfy the predicate,
it's cut off.
...
The first list contains everything the resulting list 
from takeWhile would contain if it were called with the same predicate and the same list.
...
break breaks it when the predicate is first true. Doing break p is the equivalent of doing span
(not . p).
...
only they recursively apply that to a list until there's nothing left. Observe.
...
That's kind of like having the type of 
[a], only a value of the type Maybe can contain either no elements or one element, 
...
Or zip three lists with a function that takes three parameters? Well, for that, we have zip3, zip4, etc.
and zipWith3, zipWith4, etc.
...
returning a list whose every element is a unique snowflake! The function does have a kind of strange name.
...
delete takes an element 
and a list and deletes the first occurence of that element in the list.
...
and then keep going until it finds an element that's equal to or greater than the element 
that we're inserting and it will insert it just before the element.
...
The 4 is inserted right after the 3 and before the 5 
in the first example and in between the 3 and 4 in the second example.
...
What length, take, drop, splitAt, !! and replicate 
have in common is that they take an Int as one of their parameters (or return an Int), 
even though they could be more generic and usable if they just took any type 
that's part of the Integral or Num typeclasses (depending on the functions).
...
because compare just takes two elements whose type is in the Ord typeclass 
...
man, that reads almost like real English!
...
you usually do (==) `on` something and when you're dealing with By functions that take an ordering function,
you usually do compare `on` something.



## <a name="7.3">Data.Char</a>

<img src="//s3.amazonaws.com/lyah/legochar.png" alt="挿絵 lego char" class="img-right">
...
Here's what they are:
    ...
    isControl checks whether a character is a control charact
...
Those are characters that combine with preceding letters to form latters with accents. Use this if you are French.
...
For instance, let's say we're making a program that takes a username 
and the username can only be comprised of alphanumeric characters.
...
Hmmm, well, it kind of does what words does 
...
The Caesar cipher is a primitive method of encoding messages 
by shifting each character in them by a fixed number of positions in the alphabet.
...
Decoding a message is basically 
just shifting it back by the number of places it was shifted by in the first place.
...
## <a name="7.4">Data.Map</a>

Association lists
(also called dictionaries)
are lists that are used to store key-value pairs where ordering doesn't matter.
...
filters the list so that only matching keys remain, gets the first key-value that matches and returns the value.
But what happens if the key we're looking for isn't in the association list? Hmm.
...
> instead of explicitly writing the recursion 
> because they're easier to read and identify.
...
<img src="//s3.amazonaws.com/lyah/legomap.png" alt="挿絵 legomap" class="img-left">
...
the duplicates are just discarded. This is the type signature of fromList
...
## <a name="7.5">Data.Set</a>

<img src="//s3.amazonaws.com/lyah/legosets.png" alt="挿絵 legosets" class="img-right">
...
Checking for membership, inserting, deleting, etc.
...
We can use the difference function to see which letters are in the first set 
but aren't in the second one and vice versa.
...
setNub is generally faster than nub on big lists 
but as you can see,
nub preserves the ordering of the list's elements, while setNub does not.



## <a name="7.6">Making our own modules</a>

<img src="//s3.amazonaws.com/lyah/making_modules.png" alt="挿絵 making modules" class="img-right">
...
As you can see,
we'll be doing areas and volumes for spheres, cubes and cuboids.
...
we usually export only those functions that act as a sort of interface to our module 
...
We can decide to change those functions completely or delete them in a newer version 
...
Mind the capital G. In it, we'll place three files: Sphere.hs, Cuboid.hs, and Cube.hs. 
...
Sphere.volume, Cuboid.area, etc. and each will calculate the area or volume for their corresponding object.
...
You'll be able to just import your module the next time you're writing a program 
